    ...
    def cal(self, n):
        ...
        for j in range(bcnt): #괄호의 수 만큼 재귀를 반복시키기
            ...
            for i in range(len(self.n)):
                if self.n[i] == '(': #'('의 인덱스 다 저장해가면서, ')'를 처음으로 발견하기 전까지
                    bketix_1st.append(i)
                #이런식으로 하면, (3+2)+(3+2) 이런꼴이나 (3+2*(8+9)) 이런꼴 모두 커버 가능
                if self.n[i] == ')': #제일 먼저')'의 인덱스 발견하면 나오기
                    bketix_2nd.append(i)
                    break
        #처음 나오는 ')'를 찾을때까지 모든 '('의 인덱스를 저장해 두고
        # ')'가 나오면 저장한 '('의 인덱스중 가장 큰값과 발견한 ')'의 인덱스 사이의 식을 재귀로 계산함    
